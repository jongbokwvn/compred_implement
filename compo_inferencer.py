...
def main():
    ...
    output_path_name = os.path.join(
        script_args.output_dir, "compo", script_args.subset, script_args.subreddit_name, script_args.model_name.replace("/", "-")
    )
    os.makedirs(output_path_name, exist_ok=True)
    output_file_name = os.path.join(output_path_name, f"compo_all_{local_rank}.jsonl")
