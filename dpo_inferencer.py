...
from arguments import ComPOInferencerArguments
from utils import (
    DDPManager,
    KeyWordsCriteria,
    set_seed,
...
def main():
    # parsing arguments
    parser = HfArgumentParser(ComPOInferencerArguments)
    ...
    output_path_name = os.path.join(
        script_args.output_dir, "dpo-nc", script_args.subset, script_args.subreddit_name, script_args.model_name.replace("/", "-")
    )
    os.makedirs(output_path_name, exist_ok=True)
    output_file_name = os.path.join(output_path_name, f"dpo_nc_askaliberal_{local_rank}.jsonl")
