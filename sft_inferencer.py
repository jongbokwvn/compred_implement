...
from utils import DDPManager, KeyWordsCriteria, set_seed, remove_module_prefix, rename_lora_keys
    ...
    output_path_name = os.path.join(
        script_args.output_dir, "sft-c", script_args.subset, script_args.subreddit_name, script_args.model_name.replace("/", "-") \
            if script_args.contextualize \
                else script_args.output_dir, "sft-nc", script_args.subset, script_args.subreddit_name, script_args.model_name.replace("/", "-")
    )
    os.makedirs(output_path_name, exist_ok=True)
    output_file_name = os.path.join(output_path_name, f"sft-c_all_{local_rank}.jsonl")
